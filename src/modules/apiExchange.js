...
function updatePriceElements(currency) {
  ...
  // Handle all price elements in a single loop
  document.querySelectorAll('[id^="v-calc-"]').forEach((element) => {
    ...
    if (!isNaN(originalPrice)) {
      ...
      // Handle all price elements including vehicle prices
      if (
        element.id === 'v-calc-vehicle-price' ||
        element.id.startsWith('v-calc-extra-') ||
        element.id === 'v-calc-delivery-fee' ||
        element.id.startsWith('v-calc-veh-price-p-') || // Include vehicle price radio buttons
        element.classList.contains('vehicle-value') // Additional check for vehicle value class
      ) {
